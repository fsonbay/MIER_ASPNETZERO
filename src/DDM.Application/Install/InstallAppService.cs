...
using DDM.Migrations.Seed;
using DDM.Migrations.Seed.Host;
...
namespace DDM.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : DDMAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<DDMDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{DDMConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{DDMConsts.ConnectionStringName}", constring);
        }
